...
```{r}
ci <- confint(BestModel)
exp(cbind(OR = coef(BestModel), ci))
```
...
```{r}
PorSp <-read_rds("DatosAvance2.rds") %>%dplyr::filter(!is.na(Beneficio_sp_amb))%>% 
  mutate(Ambiente=fct_relevel(Ambiente, "Urbano", "Verde", "RocaInt", "PlayaInt", "PlayaNat"))
...
sacar NA de todas las calumnas
```{r}
# Modelo general
Sp.plr <- polr(Beneficio_sp_amb ~ Relacion_MarNCP + Distancia_residenciaKm + Nota_conocimiento+  Ambiente* Especie + FrecPercibida + Conoce_especie + Conoce_nombre + Conoce_origen, data = PorSp)
